...
## Status
LIVE OP VERCEL MET DOCS (9 sep 17:35): gates 0 t/m 6 APPROVED, T001-T010 DONE. Productie https://sit-wordle-api.vercel.app (Vercel team walter-noot, project sit-wordle-api, env GAME_SECRET, secret lokaal in ~/.config/cloudly/sit-wordle.env). Smoke tegen productie 18/18. /docs is een Swagger-achtige API-referentie in SIT-huisstijl (Scalar 1.68.0 gepind met SRI op /openapi.json) met het echte SIT-logo uit de Figma brandkit in de kopbalk en als favicon met zwarte achtergrond (public/sitLogo.svg, public/favicon.svg), codevoorbeelden alleen JavaScript fetch (standaard) en curl, op verzoek Thijmen. DOMEIN GEKOPPELD (9 sep 17:05): https://wordle.svsit.nl is de canonieke URL (TXT _vercel voor eigendom, twee A-records bij Hostnet omdat een null-MX op wordle.svsit.nl een CNAME blokkeerde), smoke 18/18 op beide URLs. Geen Supabase (Thijmen: "doe zonder supabase maak het simpel"), scores in-memory.
...
- Hostnet-les: "dubbel CNAME" betekende een bestaand record van ander type (null-MX) op dezelfde naam; A-records omzeilen dat. DNS-check zonder dig: node dns.promises.Resolver met setServers op de Hostnet-nameservers.
